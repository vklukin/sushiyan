        ...
        <div class="header__nav">
            <nav>
                <ul>
                    ...
                    <li><a href="#" class="active">Запеченые роллы</a></li>
...
<main>
    <div class="container" id="grid-wrap">
        <?php 
            $cardsConnect = mysqli_query($connect, "SELECT * FROM `baked rolls`");
            $cardsCount = mysqli_num_rows($cardsConnect);
            for($i = 1; $i<=$cardsCount; $i++){
                $cardsCon = mysqli_query($connect, "SELECT * FROM `baked rolls` WHERE `id` = '$i'");
